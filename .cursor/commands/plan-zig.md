...
2. A Zig migration plan file named `<source>.zigplan.md` that thoroughly documents:
   - The exact strategy for mutating and refactoring the current TypeScript code.
   - How every external dependency will be migrated to local/self-contained code.
   - Specific implementation details for using Zod (for schema definition and validation) and header utilities (for request/response handling) within the new local codebase.
   - Step-by-step transformation plan, including before/after code structure, dependency removal, local replacements, and any required changes to types, logic, or interfaces.
...
If the processed files are located inside a subdirectory, also generate (or update) a summary file named `SUBDIR_SUMMARY.md` in that subdirectory. This file must provide a concise overview of all documented files in the subdirectory, list the generated `.zigplan.md` files, and link to them for quick navigation.
...
- Outline the Zig migration plans from all `.zigplan.md` files.
